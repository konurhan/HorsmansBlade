...
public class PlayerMovement : MonoBehaviour
{
    ...
    [SerializeField] private int timeSlice;
    private void OnAnimatorMove()
    {
        if(isGrounded)
        {
            ...
            if (Time.frameCount % timeSlice == 0)//time slicing to reduce the workload
            {
    ...
    {
        HandleJump();
        float forwardSpeed = animator.GetFloat(AnimatorController.Instance.SpeedZ);
        if (Input.GetKey(KeyCode.W) || Input.GetKey(KeyCode.S))
        {
            if (Input.GetKey(KeyCode.W))
            {
                ...
                animator.SetFloat(AnimatorController.Instance.SpeedZ, forwardSpeed);
            }
        ...
        if(x > 0)
        {
            if (animator.GetFloat(AnimatorController.Instance.SpeedZ) < speedBuff && animator.GetFloat(AnimatorController.Instance.SpeedZ) > -speedBuff) 
            {
                animator.SetBool(AnimatorController.Instance.TurnRight, true);
            }
            else
            {
        ...
        else if (x < 0)
        {
            if (animator.GetFloat(AnimatorController.Instance.SpeedZ) < speedBuff && animator.GetFloat(AnimatorController.Instance.SpeedZ) > -speedBuff)
            {
                animator.SetBool(AnimatorController.Instance.TurnLeft, true);
            }
            else
            {
                animator.SetBool(AnimatorController.Instance.TurnLeft, false);
    ...
    public void HandleJump()
    {
        if (Input.GetKeyDown(KeyCode.Space) && isGrounded)
        {
            ...
            {
                animator.SetTrigger(AnimatorController.Instance.CombatJump);
                height = combatJumpHeight;
            }
            else
            {
                animator.SetTrigger(AnimatorController.Instance.Jump);
                height = jumpHeight;
            }
