...
public class PlayerHealth : MonoBehaviour
{
    ...
    private void Die()
    {
        ...
        GetComponent<Animator>().SetTrigger(AnimatorController.Instance.Died);
        Invoke(nameof(SetIsDeadParameterTrue), 0.1f);// delayed so that take hit has some time to get triggered
        Invoke(nameof(DeactivateAnimator),3f);
    ...
    {
        GetComponent<Animator>().SetBool(AnimatorController.Instance.isDead, true);
    }
